...
(function() {
  ...
  function Calendar(options) {
    ...
    if (!this._selector(options.container)) {
      // 抛异常
      throw new Error(
        "传入的日历组件容器Selector #id不存在或者为空，请仔细检查！"
      );
      return;
    }
  ...
  Calendar.prototype = {
    ...
    initEntry: function(op) {
      ...
      if (self.options.isDebug) {
        console.log(
          "(前)初始化年月日：" + preYear + "-" + preMonth + "-" + preDay
        );
      }
      // 生成上一个月份的日历
      var outputs = [];
      self.refreshData(
        ...
        function(output) {
          ...
          var curMonth =
            options.month || self.tod(self.DateObj().getMonth() + 1);
          ...
          if (self.options.isDebug) {
            console.log(
              "(中)初始化年月日：" + curYear + "-" + curMonth + "-" + curDay
            );
          }
    ...
    refreshData: function(dateObj, callback) {
      ...
      // 获取星期
      var tmptmp = new Date(
        Date.parse(self.nowYear + "/" + self.nowMonth + "/01")
      );
      ...
      var preDayCount = self._judgeDaysByYearMonth(
        self.nowYear,
        parseInt(self.nowMonth - 1)
      );
        ...
        for (
          ...
          tmpInfo.push({
            ...
            date:
              self.nowYear +
              "-" +
              self.tom(self.nowMonth) +
              "-" +
              self.tod(indexindex), //完整日期
            isforbid: "1" //当前月可点击
          });
      ...
      for (var i = 0, len = dataObj.tmpInfo.length; i < len; i++) {
        ...
        if (!templData) {
          ...
          if (value.date == curr) {
            ...
            if (value.isSelected) {
              template =
                '<div class="em-calendar-item em-calendar-active em-calendar-curdate  isforbid{{isforbid}}" date="{{date}}"><span class="day">{{day}}</span><p class="lunar">今天</p><span class="dot dot-type1"></span></div>';
            } else {
              template =
                '<div class="em-calendar-item em-calendar-active em-calendar-curdate  isforbid{{isforbid}}" date="{{date}}"><span class="day">{{day}}</span><p class="lunar">今天</p></div>';
            }
          } else {
            if (value.isSelected) {
              // 个性化和业务贴近
              template =
                '<div class="em-calendar-item  isforbid{{isforbid}}" date="{{date}}"><span class="day">{{day}}</span><p class="lunar">{{lunar}}</p><span class="dot dot-type1"></span></div>';
            } else {
              template =
                '<div class="em-calendar-item  isforbid{{isforbid}}" date="{{date}}"><span class="day">{{day}}</span><p class="lunar">{{lunar}}</p></div>';
            }
          }
    ...
    _addEvent: function() {
      ...
      self.mySwiper = new Swiper("#swiper-container", {
        ...
        on: {
          ...
          slideNextTransitionStart: function() {
            ...
            if (self.options.isDebug) {
              console.log(
                "(后)初始化年月日：" + nextYear + "-" + nextMonth + "-" + "01"
              );
            }
            ...
            if (self.tom(currMonthIndex) !== self.curMonth) {
              ...
              document
                .querySelector(".em-calendar-curdate")
                .classList.add("em-calendar-active");
            }
          },
          /**
      ...
      if (self._selector(self.options.backToToday)) {
        self
          ._selector(self.options.backToToday)
